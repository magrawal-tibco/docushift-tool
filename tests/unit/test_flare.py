...
def run(tmp_path: Path, files: Mapping[str, str], *, api_roots: tuple[str, ...] = (),
        product_name: str = "", publish: bool = False) -> Run:
    """Converts every output root in a tree, the way the driver does.

    `publish` hands the context the URL map the driver builds in 6e. Off by default,
    which is also production's state for a version with no API tree: an empty map
    means the rewrite does not fire, not that it fires with a broken address.
    """
...
def test_a_link_into_an_api_tree_becomes_its_published_url(tmp_path: Path) -> None:
    """456 Flare references across 30 versions, and `ftl`'s roots sit inside `html/`.

    The target is a real page this run deliberately does not convert, so the
    pre-6e answer was to drop the link and report it. It is the same file the
    `-resources` copy publishes, and the URL says so.
    """
    files = basic()
    files["html/Content/intro.htm"] = topic(
        "Introduction",
        '<p>See <a href="../api/javadoc/index.html#method-summary">the API</a>.</p>',
    )
    files["html/api/javadoc/index.html"] = "<html><!-- Generated by javadoc --></html>"
    files["html/api/javadoc/index-all.html"] = "<html></html>"

    result = run(tmp_path, files, api_roots=("html/api/javadoc",), publish=True)

    assert (
        "[the API](https://docs.example.com/en-us-tib-x-userdocs-resources"
        "/en-us/prod/api-references/1-0-0/javadoc/index.html#method-summary)"
    ) in result.body("Content/intro.md")
    assert "TOPIC_LINK_DANGLING" not in result.codes()
    assert result.context.api_links == 1


def test_a_link_into_an_api_tree_still_dangles_when_nothing_is_published(
    tmp_path: Path,
) -> None:
    """`--input` and every non-primary locale: an empty map drops, never guesses."""
    files = basic()
    files["html/Content/intro.htm"] = topic(
        "Introduction", '<p>See <a href="../api/javadoc/index.html">the API</a>.</p>'
    )
    files["html/api/javadoc/index.html"] = "<html><!-- Generated by javadoc --></html>"

    result = run(tmp_path, files, api_roots=("html/api/javadoc",))

    assert "the API" in result.body("Content/intro.md")
    assert "](" not in result.body("Content/intro.md").split("the API")[1][:2]
    assert result.codes()["TOPIC_LINK_DANGLING"] == 1
    assert result.context.api_links == 0


def test_a_path_inside_an_api_tree_that_is_not_on_disk_stays_dangling(
    tmp_path: Path,
) -> None:
    """`apiref.py`'s rule on the link side: a marker decides and a name never does.

    Emitting a URL for a page the copy will not contain trades a reported failure
    for a silent 404, which is the one trade this tool does not make.
    """
    files = basic()
    files["html/Content/intro.htm"] = topic(
        "Introduction", '<p>See <a href="../api/javadoc/Gone.html">the API</a>.</p>'
    )
    files["html/api/javadoc/index.html"] = "<html><!-- Generated by javadoc --></html>"
