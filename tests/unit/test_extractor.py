...
def docbook_page(stylesheet: str, body: str = "text") -> str:
    """A page carrying the generator comment DocBook XSL writes into every head."""
    head = (
        "<!-- Generated by DocBook XSL Stylesheets V1.76.1 -->"
        f'<link rel="stylesheet" href="{stylesheet}" type="text/css"/>'
    )
...
def test_a_docbook_root_is_where_the_stylesheet_link_resolves(tmp_path: Path) -> None:
    """A `str` version ships eight byte-identical duplicates of a guide under `html/`.

    They hold DocBook pages too, so "holds a DocBook page" selects nine roots and
    converts every duplicated guide twice. What separates them is where the
    stylesheet link lands: `html/` owns its `css/`, the duplicates borrow one that
    is not there (`architecture.md` §5.6.3).
    """
    tree = build_tree(
        tmp_path / "db",
        {
            "html/index.html": docbook_page("css/sbhelp.css"),
            "html/css/sbhelp.css": "body{}",
            "html/authoring/topic.html": docbook_page("../css/sbhelp.css"),
            "authoring/topic.html": docbook_page("../css/sbhelp.css"),
        },
    )

    roots = find_output_roots(tree, SourceEngine.DOCBOOK)

    assert [root.relative_to(tree).as_posix() for root in roots] == ["html"]
...
def test_an_engine_with_no_root_rule_reports_none(tmp_path: Path) -> None:
    ...
    tree = build_tree(tmp_path / "none", {"ch01.html": page()})

    assert find_output_roots(tree, SourceEngine.ROBOHELP) == []
