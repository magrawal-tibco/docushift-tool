...
def test_a_link_into_an_api_tree_becomes_its_published_url(tmp_path: Path) -> None:
    """2,581 of the corpus's 3,194 -- 81% of the traffic in 7% of the versions.

    `amx-bpm` files three API trees inside `bpmhelp/`, which is also the doc-set,
    so the reference resolves inside the engine's own output root and is dropped
    for not being a converted topic. That is the branch this check precedes.
    """
    result = run(tmp_path, {
        "doc/html/GUID-AAA1.html": topic("GUID-AAA1", "Getting Started", (
            '<p>See <a href="../javadoc/index.html#field-detail">the API</a>.</p>'
        )),
        "doc/javadoc/index.html": "<html><!-- Generated by javadoc --></html>",
        "doc/javadoc/index-all.html": "<html></html>",
        **{f"doc/html/{k}": v for k, v in toc(node("GUID-AAA1", "Getting Started")).items()},
    }, api_roots=("doc/javadoc",), publish=True)

    assert (
        "[the API](https://docs.example.com/en-us-tib-x-userdocs-resources"
        "/en-us/prod/api-references/1-0-0/javadoc/index.html#field-detail)"
    ) in result.body("getting-started.md")
