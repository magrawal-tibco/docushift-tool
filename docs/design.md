...
**`metadata.yml` is validated on its two keys** (§10). `nav.yml` no longer exists and `meta.yml`'s placeholder shape is gone: the AEM contract of 2026-09-10 fixed `csg-product` at product level and `csg-version` at version level, so there *is* a field list now and the file is checked against it rather than for well-formedness alone. `toc.yml`, `index.md` and `csh.yml` are validated on their content as specified above and in §9.6.
