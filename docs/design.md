...
**Navigation synthesis is built (Phase 6a), and so are three of the distributor's four steps.** Synthesis assembles one version's node list out of what the engines reported and writes `toc.yml`, `metadata.yml` and any generated landing or container pages from the Jinja templates in `config/aem_templates/`. The distributor that copies each version's output into the target publishing layout landed in 6b for `online-help` (§10.3) and in 6c for the three document doc-classes (§10.4, §10.5); the `-resources` tree and the cross-boundary rewrite (§10.6, §10.7) are still specified only. As synthesis is built it runs **inside `docushift convert`, before the build-and-swap** — the node list exists only while the engine's units are in hand, and the pages it generates must reach the staging tree. `converter/navigation.py`, and `architecture.md` §6.5 for the three cross-unit rules the corpus forced.
