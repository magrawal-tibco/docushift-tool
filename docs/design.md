...
**Navigation synthesis is built (Phase 6a); the distributor is not.** Synthesis assembles one version's node list out of what the engines reported and writes `toc.yml`, `metadata.yml` and any generated landing or container pages from the Jinja templates in `config/aem_templates/`; the distributor that copies each version's output into the target publishing layout is still specified only. As built it runs **inside `docushift convert`, before the build-and-swap** — the node list exists only while the engine's units are in hand, and the pages it generates must reach the staging tree. `converter/navigation.py`, and `architecture.md` §6.5 for the three cross-unit rules the corpus forced.
