...
### 8.4 Link, asset and CSH integrity — **Built** (Phase 7b)

`docushift validate`, in `validation/`. The only command that gates, and the only one that reads the **published tree** rather than the database: `tree.py` enumerates what is on the shelf, and `links.py`, `artifacts.py` and `csh.py` are pure functions over one version folder that take no `CatalogManager`, no `StateStore` and no `FindingsRun`. `driver.py` is the only thing that records. A published tree outlives the row that produced it, so the selectors filter directory names and a target another machine synced is validated with no `versions.csv` agreement required (`architecture.md` §7.5).

**The premise: almost everything it looks for should not be there.** §6.4 resolves the copy and the link in one pass, so under invariant 13 a broken relative asset link is not a discovery about the corpus — it is a regression in this tool. A linter written to *find* problems would be tuned for recall and would ship with a noisy baseline; this one is written to **stay** near zero, which is why every gating check is one whose clean answer is knowable in advance, and why a check that cannot make that claim is a warning. Measured over a real published tree of 91 version folders and 40,054 references on 2026-09-16: **two broken links, both upstream authoring**, and they were only visible once three classification rules stopped reporting 121 correct links as broken.

**Links are classified before they are checked, through the same `transforms/links.py` the emitter used.** One resolution, two callers — §5.5.6 measured what a second implementation would cost: 1,872 WebWorks references reported missing that are not, 1,224 percent-encoded and 648 backslash-separated. A relative link resolves against the filesystem and a missing target is an error. An absolute URL is external — which, after Stage 7, includes every rewritten API-reference link (`architecture.md` §6.4) — and is skipped and counted by default, requested over HTTP only behind `--check-external`, deduplicated once per URL for the whole run.

**A reference whose raw first segment names a published tree is a host-less URL and resolves against the target root.** With `publish_base_url` empty — the shipped state — a rewritten API link is a tree-rooted path with no host (`architecture.md` §6.4.2), which `classify()` rightly calls relative; resolve it against the citing page's own directory and it lands nowhere. **121 of the sample's 123 "broken" links were exactly that mistake.** The test runs on the raw path, before resolution, because afterwards the tree name is buried behind the page's directory. Resolving rather than merely skipping is the bonus: it catches help pointing into an API tree nobody synced.

**Resolution is case-sensitive on every platform**, because the target is published to Linux. This is not a hypothetical: on its first run it found three `toc.yml` paths differing from their files only in case, and the cause was worse than a 404 — `navigation._free` tested a generated container page's path case-sensitively, so on Windows the page was written *into* the converted topic and three topics lost their text with nothing in the run reporting it.

**A missing file is an error; a missing anchor is a warning.** 1,626 of the sample's 14,055 fragments do not resolve — 11.6% — and an 11.6% rate cannot gate. They are still raised: spot-checks say these are anchors the conversion genuinely dropped, which makes `ANCHOR_MISSING` the first measurement of a defect nobody had counted. Explicit `id=`/`name=` attributes count as anchors beside computed heading slugs, since the engines emit 11,887 of them.

**Reference extraction reads the HTML inside the Markdown, and is deliberately not CommonMark-correct.** 2,520 of 40,054 references are raw `<a href>`/`<img src>` in the passthrough the engines emit for tables GFM cannot express; a reader seeing only `![](…)` calls the tree clean while those images 404. Fenced blocks and code spans are masked — 0 references found inside them, kept as insurance — but four-space indentation is *not* treated as code, because in converted help it is list continuation and honouring it would silently stop checking 953 real references.

**Asset links are checked, and finding one is a regression rather than a discovery.** `validate` re-resolves them because an invariant nobody tests is an assumption. It does **not** report unreferenced assets as errors — orphans are a Stage 5 report line (§6.4 step 7), and 54.6% of Flare's images are orphans by design of the authoring tool, not by defect.

**The four AEM artifacts get field-level checks, and only `metadata.yml` is required.** The 2026-09-10 contract fixed `csg-product` at product level and `csg-version` at version level, so there is a field list now and the file is checked against it rather than for well-formedness alone. `toc.yml` paths are links and get `LINK_BROKEN`; `version.yml` is checked for rows naming no folder, folders with no row, duplicates and numeric-descending order, all as warnings because `sync` deliberately preserves rows it does not own; `csh.yml` and topic frontmatter are checked against each other in both directions (§9.6). Everything except `metadata.yml` is checked *if present* and never demanded — a global required-set would raise around sixty findings against correct output, since `online-help` carries `index.md` in 2 of 17 folders and `csh.yml` in 4. A file that will not parse is `ARTIFACT_UNPARSED` and the rest of that file's checks are skipped, because continuing would make every further finding about it unsound.

**`api-references/` is not link-checked and `validate` writes nothing.** The API trees are copied Javadoc, not this tool's output; their `metadata.yml` is ours and is checked, and the rest is somebody else's generator. There is no `--fix`: the published tree is regenerated by `sync`, so a repair applied here would be reverted by the next run.
...
**Navigation synthesis is built (Phase 6a), and so are three of the distributor's four steps.** Synthesis assembles one version's node list out of what the engines reported and writes `toc.yml`, `metadata.yml` and any generated landing or container pages from the Jinja templates in `config/aem_templates/`. The distributor that copies each version's output into the target publishing layout landed in 6b for `online-help` (§10.3) and in 6c for the three document doc-classes (§10.4, §10.5); the `-resources` tree landed in 6d (§10.6) and the cross-boundary rewrite's first class in 6e (§10.7), the latter inside `convert` rather than `sync`; §10.7's class 2 is still specified only. As synthesis is built it runs **inside `docushift convert`, before the build-and-swap** — the node list exists only while the engine's units are in hand, and the pages it generates must reach the staging tree. `converter/navigation.py`, and `architecture.md` §6.5 for the three cross-unit rules the corpus forced.

**The placeholder templates are gone, and the guess with them** (AEM contract, 2026-09-10). The 2026-09-09 position was that `nav.yml` and `meta.yml` had no authored spec and would stay as Phase-1 scaffolding until one arrived. The spec arrived and named neither: **`nav.yml` is deleted** — nothing consumed it, nothing implemented it, and `toc.yml` was always the specified navigation artifact — and **`meta.yml` became `metadata.yml`** carrying exactly `csg-product` at product level and `csg-version` at version level. The ~11 invented fields were deleted rather than kept alongside, because a guess shipped beside a contract reads as part of it. The one grounded input — SuiteHelp's `GUID-*-homepage.html` supplying `publication-title`, `release-version` and `release-date` in all 314 doc-sets that ship one (`architecture.md` §5.2.7) — is **still collected but no longer emitted**: it is a cross-check against the catalog's version and release year, and a disagreement is a `METADATA_MISMATCH`, which moved from `sync` to `convert` because that is the only stage where the figures exist.
...
**Navigation synthesis does not just serialize what the engine handed it.** Four rules apply to the node list before it is written, three of them settled by the 2026-09-09 Flare survey (`architecture.md` §5.1.5) and all engine-neutral — two create nodes the source does not supply, one moves nodes the source misfiles. They run **after** the version's units have been assembled into one tree, not per unit, so "last two nodes" means last in the version (`architecture.md` §6.5):
...
- **What's New is the second node, where the page says anything** (Phase 11a, 2026-09-17). 648 of 903 Flare roots ship one under `_templates/` and 481 of those are real; the other 167 are the authoring template shipped unfilled and are not converted at all. Of the 481, 407 are already filed somewhere inside a guide and are moved to second, and 74 are in no TOC entry and are inserted there. The landing page keeps first place because it is where a reader arrives; What's New is what a reader on an upgrade came for, and its position stops depending on what each writer happened to do. Where no unit reported a landing page the node goes first and the generated version index lands above it, so it is second either way. `architecture.md` §5.1.5 for the detection rule and why it needs no threshold.
...
- **The support and legal pages are the last two nodes** — `Documentation and Support Services` second-last, `Legal and Third-Party Notices` last, both at top level. The engine reports which converted topic is which; the synthesizer **moves** those nodes to the tail rather than appending, because they are already TOC entries in 97% and 96% of Flare roots and appending would duplicate the topic. It is the landing-page rule inverted, and it mostly ratifies the source: 610 of 676 roots (90.2%) already end support-then-legal. Three constraints follow from the survey. The label is taken from the node, never hard-coded — the support page's heading is `TIBCO` in 565 roots, `ibi` in 49 and `Spotfire` in 45. There is **one legal node, not two** — no separate third-party-notices page exists in the corpus, and the combined page has a single `h1` and no `h2` sections. And a version that ships neither page simply has a shorter tail; unlike a headless container, a missing legal page breaks nothing, so nothing is generated to stand in for it. **Across units the choice is positional, not content-addressed** (measured 2026-09-12): 105 of 400 multi-book versions carry a legal page in more than one book and 83 of those sets are not byte-identical, but the only difference is FrameMaker anchor ids, so a hash would keep all nine copies of one notice. The first unit in engine order supplies the tail; the rest leave the navigation as `NAV_NODE_DROPPED` and stay on disk.
...
- **The drop-downs legitimately disagree**, as of 6c: `user-guides` carries versions that shipped no converted help. That is the point of the per-doc-class placement, not a defect to reconcile, and 6b's assembly rule needed no change to produce it — it already takes the present set from the directory listing.

### 10.4 Step 2, the document router — **Built (Phase 6c)**

Which doc-class a non-converted document lands in. Grounded in a 2026-09-07 survey of 1,822 versions and 11,633 documents, re-grounded on 2026-09-15 (`architecture.md` §6.2.1). Built as `sync/router.py` and verified against the whole corpus end to end (`architecture.md` §6.7).

**Locate the two source folders.** For a version's extracted root `V`, the PDF folder is `V/pdf` or `V/doc/pdf`, and the document folder is `V/doc` **or `V/doc/doc`** — the nesting applies to both members of the pair, and root wins in both. Both depths occur: 1,123 versions put `pdf/` and `doc/` at the root, 373 nest them under `doc/`. Check both; a version may have either, both, or neither. *Corrected 2026-09-15:* this paragraph originally located only `V/doc`, which leaves **676 files in 346 versions across 120 products** unrouted — chiefly `.txt` readmes and reminder notices, 376 of them `release-information`.

Only *files directly inside* these folders are routed here: subdirectories are the converter's and the API router's business, not this step's. `doc/relnotes/` is the case that tests the rule and confirms it — 12 versions have one and it holds `Default.htm`, `csh.js`, `.mcwebhelp` and `.mclog`, a Flare output root rather than a folder of release notes.

**A file an engine output root owns is not a document.** Before routing, resolve the version's output roots with `engines.roots.find_output_roots` and skip any file `owning_root` claims. One product in the corpus ships its Flare help *as* `doc/` (`flogo-oracledb/1.2.1`), so its `Default.htm`, `csh.js`, `Default.mcwebhelp` and `tp-html.mclog` sit exactly where the router looks. This is the precedence `extractor/inventory.py` already applies when it tests `owning_root` before the router directories; applying it here keeps a single file from being an output-root file in the inventory and a published document in the tree.
...
- **`\b` is not a usable boundary here.** `_` is a word character, so `\b` does not match between `_` and `rel`, and a first pass written with it misrouted **1,753 `*_relnotes.pdf` files into `user-guides`**. The alternation therefore lists `relnotes` bare and anchors the spelled-out form on `(^|[\s_.-])`. *Corrected 2026-09-15:* that count covered the PDF folder only. The same anchor also misses **1,108 files in the document folder**, filing a readme as a reference document — **2,921 in total** — so the regression test must exercise both folders.
...
**What the corpus publishes that is not a document: six files.** After the output-root skip, `reference-documents` receives `context.xml`, `t1.aspx` ×2 and `rtview`'s chrome pages. They are accepted rather than filtered by extension, because the same deny-list would drop the corpus's five genuine HTML release notes and a `license.htm` — six strays across 1,822 versions is the cheaper error, and `reference-documents` is the catch-all by design.

### 10.5 Step 3, the document doc-class index — **Built (Phase 6c)**

Each of `user-guides`, `release-information` and `reference-documents` gets a flat `index.md`, a `toc.yml` and a `metadata.yml` built from its routed file list, because a copied PDF with no index is unreachable. Grounded in a 2026-09-08 survey of the same 1,822 versions, re-measured on 2026-09-15 (`architecture.md` §6.2.2). Built as `sync/documents.py`, placed by `sync/distributor.py:sync_documents`.

**Input** is §10.4's routed list for one version, **de-duplicated by lower-cased filename within each doc-class**, the root folder winning over the nested one in both pairs — `V/pdf` over `V/doc/pdf`, `V/doc` over `V/doc/doc`. 25 versions carry both PDF folders and share **178 filenames**, and 24 carry the same readme at both document depths; without this the file is copied and listed twice. The corpus publishes **12,124 files** after de-duplication of the 12,352 routed: 5,007 `user-guides`, 3,824 `reference-documents`, 3,293 `release-information` (measured as built, `architecture.md` §6.7).
...
2. **The PDF's Info-dictionary `/Title`**, read with **`pypdf`**, for `.pdf` files that reached step 2 — i.e. `user-guides`, which is by construction the residue of the step-1 patterns. Over all 5,007 de-duplicated user-guide PDFs, re-measured with pypdf on 2026-09-15: **70.8% usable (3,545), 27.9% blank (1,396), 1.2% junk (59), 7 unreadable**. **Wrap the read in `except Exception`** — not in a pypdf error type: of the seven failures only `TIB_mftcc_8.4.4_user_guide.pdf` raises `PdfStreamError`, while `TIB_smap_1.0.0_install_guide.pdf` and `silver-mobile`'s two guides (recurring across versions) raise a bare `ValueError`. A failure falls through to step 3 exactly as a blank does, and raises a `DOCUMENT_UNREADABLE` note; a **blank is never reported**, because at 27.9% it is the corpus's normal state. **Never use a byte-level `/Title` regex** — outline bookmarks are `/Title` entries too and usually precede the Info dictionary, so a regex returns `'Basic Tab'` and `'Table of contents'` while claiming 57% success.
   - **Reject as junk**, falling through to step 3: `untitled` case-insensitively, any title ending `.book`/`.fm`/`.doc`/`.docx`/`.pdf`/`.indd`/`.mif`, and any beginning `Microsoft Word - `. That is the whole of it — every junk title in the corpus is the authoring tool's source filename, **59 of them in 35 distinct values**.
   - **`pypdf` over `pymupdf`, verified not assumed** (2026-09-09): head to head over all 5,007, byte-identical titles on the files both read (100.00%), pypdf 4× faster (122 s v 501 s). pymupdf is AGPL; pypdf is BSD-3-Clause. The seven pypdf-only failures cost **one usable title**: MuPDF recovered a title from six of them and all six were junk, rejected by the filter above regardless.
...
**Currency is checked before anything is built**, which is the one place these doc-classes do not reuse `online-help`'s shape. §10.3 stages the tree and then compares it, which is affordable for Markdown; here the staging copy is hundreds of megabytes of PDFs and the comparison would follow thirteen minutes of re-opening them. So `_documents_current` compares the *routed* filename set against the published one — plus `index.md`, `toc.yml` and `metadata.yml` — and then size-and-mtime per file, before a title is read. The inference it rests on is that the three rendered files are a pure function of the routed filenames and the PDFs' contents. A **template** change is the one thing that escapes it, and `--force` is the answer, as it is to a changed engine.
...
### 10.7 Step 4, the cross-boundary rewrite — **Classes 1 and 3 built (Phase 6d, 6e); class 2 remains Phase 7**

> **Class 1 did not turn out to be Stage 7 work at all.** The targets are real — re-measured 2026-09-15 through the shipped classifiers, **3,194 references resolve into an API root in 41 of 49 in-scope versions** — but none of them reached Markdown: each engine's `link()` dropped a topic-looking href that is not a converted topic. A Stage 7 pass over the placed tree would have found nothing and reported success. So the rewrite moved to **conversion time** in 6e, where the reference still exists: the driver hands `ConversionContext` a resolved root-to-URL map built through the same `apirefs.select()` Stage 7 publishes with, and four engines each gained the same three-line check. The engines still know nothing about the publishing layout — they are told. Class 3 shipped in 6d. **Class 2 stays here and stays Phase 7**, for the reason class 1 turned out not to have: its destination needs §10.4's router, which is Stage 7 knowledge. Full trace, the measured distribution and two corrections to 6d's cost estimate in `architecture.md` §6.4.2.
...
**1. Links into `-resources` — built, Phase 6e, at conversion time.** Converted help links into an API-reference tree (`[…](api/java/index.html)`) become the published URL of the sibling repository, composed by `sync/apirefs.published_url()` — the same function that names the folder the copy is placed in, so a link cannot point at a repository or a folder that was never created. An unset `publish_base_url` yields the tree-rooted path with no host rather than dropping the link. §8.4 does not resolve these against the citing page's directory — it recognizes the tree name on the raw path and resolves them against `--target-dir` instead, which turns the rewrite into a check that the API tree was actually synced. Measured over the cache: **2,854 rewritten in 38 versions**, of 3,180 references — 2 targets the package does not ship, and 324 that a `<code>` span had already flattened before any engine saw them (`architecture.md` §6.4.2).

**2. References that escaped the output root**, handed over by §6.4 step 3.5 with their resolved absolute source path. **Still Phase 7.** Route the target through §10.4's document router to find the doc-class it landed in, then emit a relative path across doc-classes — both are in the docs repo, so this stays a filesystem link and stays checkable. Measured: 279 in the Flare sample (76 into `doc/`, 66 into `pdf/`, 68 at the version root, 15 in `license/`), **0 in DITA and 0 in WebWorks** (`architecture.md` §5.5.8). Where the target resolves to no routed document — 152 of the 279, references to files the ZIP does not ship — emit the link text as plain text and count it. `DOC_REFERENCE_MISSING` is registered for this and is the one `Stage.SYNC` code no shipped path raises.

**3. The `archives/` and doc-class index links** — built, Phases 6c and 6d, which §10.5 and §10.6 generate from lists rather than rewrite.

**Nothing is re-resolved from a URL or a cache path.** Every rewrite here consumes a target that Stage 5 already resolved and recorded; Stage 7 changes the *form* of a reference, never its identity. This is the rule `architecture.md` §5.5.9 measures the cost of breaking — 7,231 broken image links in the predecessor's output, from a second derivation of a path that was already known. 6e keeps it by moving earlier rather than by deriving twice: class 1 is rewritten at the moment the reference is resolved, which is the only moment it exists.

**Class 2 is last because it needs both destinations on disk, and it is in Stage 7 because conversion does not know the publishing layout.** That sentence was written of all three classes and survives for one. It was never a statement about *where the reference lives* — which is the converter's problem — only about *where the answer comes from*, and class 1's answer turned out to be cheap to hand over.
...
| 8.4 | Link, asset, anchor, artifact and CSH integrity over a published tree — the one gating command | Built | `validation/` (`tree.py`, `references.py`, `links.py`, `artifacts.py`, `csh.py`, `driver.py`); `architecture.md` §7.5 |
...
| 10.7 | Cross-boundary link rewrite (`-resources` URLs, escaped asset references) | Built (classes 1, 3) | Class 1 moved to **conversion time** in 6e — a Stage 7 pass would find nothing to rewrite. Class 2 (279 Flare escapes into the document doc-classes) stays **Phase 7**: it needs §10.4's router |
