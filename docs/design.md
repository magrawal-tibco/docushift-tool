...
**Navigation synthesis does not just serialize what the engine handed it.** Three rules apply to the node list before it is written, all settled by the 2026-09-09 Flare survey (`architecture.md` §5.1.5) and all engine-neutral — two create nodes the source does not supply, one moves nodes the source misfiles. They run **after** the version's units have been assembled into one tree, not per unit, so "last two nodes" means last in the version (`architecture.md` §6.5):
