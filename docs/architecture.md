...
flowchart TD
    ...
    subgraph AEM["AEM Architecture Synthesis"]
        ...
        AEM1 --> AEM2["toc.yml, metadata.yml, version.yml\nLanding Pages & Frontmatter"]
    end
...
**A tree can be measured without its package.** The five columns are written from a walk of the extracted tree, and the ZIP is wanted only for the checksum that decides whether that tree is current. Where the packages have been cleaned away but the trees remain — the state six converted DataSynapse versions were found in on 2026-09-18, extracted before `record_extract_inventory` existed — `extract` returns `NO_PACKAGE` before it can reach the walk, and the columns stay blank permanently. `docushift extract --measure-only` walks what is on disk instead: no package read, no tree written, outcome `measured` counted apart from `extracted`. It deliberately writes **no `extract_zip_checksum`** (a fabricated one would tell the next real extract that an unverified tree is current) and **does not touch `version_state`** (these versions may already have converted). The columns then assert what the tree contains, which is all they ever asserted; what the flag gives up is the claim that the tree matches upstream, which is why it is opt-in and refuses to run beside `--force`.
...
**The blocks that unroll are mostly the website's, not the product's, and they are cut** (Phase 11b, measured 2026-09-17 over 778 landing pages). Once unrolled, `Release Documents`, `Related Product Documentation`, `Most Visited Topics`, `Downloadable PDF Guides`, `Videos`, `Key Guides` and `Recommended Topics` are `docs.tibco.com` furniture — a PDF shelf, a site-wide popularity list, links to sibling products — whose targets are exactly the 5,911 non-HTML and 3,698 absolute links counted above. In a published version folder they render as bare text lists. The page is therefore truncated at the **first** of those labels, which keeps the product overview and `Key New Features` and drops the rest: **688 pages trimmed, 90 untouched**.

Cut at the labels rather than at "the section after Key New Features", because the obvious rule fails twice: **351 of the 778 have no Key New Features section** and would go untrimmed, and **10 carry genuine feature subsections below it** — `Server Improvements`, `Governance & Security`, `Platform Support` — which anchoring would delete. Over all 778 there is no page whose Key New Features section the label rule removes. It is applied to the converted Markdown rather than the HTML, because in the HTML these labels are frequently not headings: `datasynapse`'s landing page has one `h1` and no `h2` at all.

**What's New is the second node, where it says anything** (Phase 11a). 648 of 903 roots ship a What's New page under `_templates/`, and the filename is not stable — `Whats-New.htm` dominates, with `Whats-New.html` (12), `What_s-New.htm` (9), `Whats_New.htm` (6) and `What's-New.htm` (5) behind it, so the stem is folded to alphanumerics before the match. The per-component variants (`-old` 5, `-client` 4, `-server` 4) are deliberately not matched: which of a pair is *the* What's New is a judgement the filename does not settle.

**167 of the 648 are the authoring template shipped unfilled**, their whole body bracketed instructions — *"[Provide list of update made to the product documentation…] [Feature Name] [Use sections with h2 styles…]"*. Detection strips every `[...]` span, the navigation chrome and the leading heading, then counts the alphanumeric residue. Across 654 files that residue is **0 for 167 and 60+ for 467, with twenty between 20 and 59 and nothing at all between 1 and 19** — the twenty are genuine short releases ("No new features have been added in this release."). A cutoff anywhere in that gap behaves identically, which is what makes `== 0` a measurement rather than a tuning. Those 167 are not converted, and the note is `WHATS_NEW_PLACEHOLDER`.

The remaining 481 are hoisted to second — after the landing page, which keeps first place because it is where a reader arrives. **407 of them are already somewhere in the source TOC**, usually buried inside a guide, and are moved rather than copied; **74 are in no TOC entry at all** and would otherwise be discarded as `unreferenced-template`, so the planner declares them referenced instead of special-casing the rejection. Only **4 versions** file a *placeholder* What's New in their navigation, and that is the one place this rule overrules the source.
...
- **`GUID-*-homepage.html` → `meta.yml`, not a topic.** It is present in 314 of 353 doc-sets and **all 314 carry `publication-title`, `release-version` and `release-date`**; its entire `<article>` is `<div class="titles">` holding those three divs and no body content. It is the only in-package source for the published release date and title. It **fed `meta.yml` until the AEM contract arrived (2026-09-10) and named neither field**; since Phase 6a it is read as a cross-check against the catalog's version and release year — a disagreement is a `METADATA_MISMATCH` — and is then skipped. `publication-title` is deliberately not compared: a version ships up to 11 publications, so equality with one display name would fire on almost all of them. 5 doc-sets ship more than one (take the one whose GUID matches the TOC root); 39 ship none.
...
- **Cross-version regression — built in Phase 7c** (`design.md` §8.7, §7.6 below). Identifiers present in the previous converted version and absent from this one are reported, against the **published tree** rather than against a batch, because the shelf is where the upgrade path is. A dropped identifier is an upgrade that breaks the product's Help button, and it is invisible from within a single version: over the cache, **51 of 317 comparable version pairs (16.1%) drop at least one**, 69.6% of them across a major version.
...
└── en-us/
    └── tibco-ems/
        ├── metadata.yml                # csg-product: TIBCO Enterprise Message Service
        ├── online-help/
        │   ├── version.yml             # the drop-down, listing only the versions THIS doc-class has
        │   └── 10-4-0/…                # converted Markdown + toc.yml, metadata.yml, index.md, csh.yml
        ├── user-guides/                # version.yml + 10-4-0/ of user-guide PDFs, index.md, toc.yml, metadata.yml
        ├── release-information/        # version.yml + 10-4-0/ of release notes and readme, ditto
        └── reference-documents/        # version.yml + 10-4-0/ of VPAT, licence, remaining doc/ files, ditto
...
The three document doc-classes are filled by **the folder a file came from, then its name**. Measured 2026-09-07 over the extracted cache — 1,822 versions, 11,633 documents in `pdf/` and `doc/` (script `C:\tmp\an_docclass*.py`), and re-measured 2026-09-15 before Phase 6c was built (`C:\tmp\6c_router*.py`). The re-measure reproduced every count below and corrected three rules; the corrections are marked where they apply:
...
**Both folders appear at two depths, and the document folder's nested form is easy to forget.** 1,123 versions carry `pdf/` and `doc/` side by side at the package root, but 373 nest them one level down as `doc/pdf/` **and `doc/doc/`**. The locator checks both depths for both folders; treating the root layout as the only one would silently publish nothing for a fifth of the corpus. *Corrected 2026-09-15:* the first written specification (`design.md` §10.4) carried the shift for the PDF folder and dropped it for the document folder, locating `V/doc/pdf` but only `V/doc`. The measured cost of that asymmetry is **676 files in 346 versions across 120 products** — mostly `.txt` readmes and reminder notices, splitting 376 `release-information` / 300 `reference-documents` and 0 `user-guides`, since the nested PDFs were already reached. `V/doc` is empty in 346 of the 370 versions that have `V/doc/doc`, and in the other 24 it holds the same filenames one level up, which the de-duplication rule below already resolves root-first.

**Only files directly inside these folders are documents, and the corpus pays one file for the rule.** 117 document-extension files sit one level further down in a directory that is not `pdf`, `doc` or `html` — **116 of them in `ebx-addon`**, out of scope since Phase 3.5, leaving a single file in `stat`. The one subdirectory name that looks like it should qualify, `doc/relnotes/` (12 versions), contains `Default.htm`, `csh.js`, `.mcwebhelp` and `.mclog`: it is a **Flare output root**, and therefore Stage 5's, exactly as the rule intends. For the same reason **a file an engine output root owns is never routed** — `flogo-oracledb/1.2.1` ships its Flare help *as* `doc/` — which is the precedence `extractor/inventory.py` already applies when it counts a file as output-root before testing it against the router directories.

Resulting split, **before de-duplication**: **5,088 `user-guides` (55.7%), 2,214 `reference-documents`, 1,829 `release-information`** from `pdf/`; 1,343 / 1,159 from `doc/`. Every one of the 11,633 files routes — the classification has no "unknown" bucket, because `user-guides` is the default rather than a match. **After** the §6.2.2 de-duplication the corpus publishes **12,124 files** (5,007 `user-guides` / 3,824 `reference-documents` / 3,293 `release-information`), of which 676 arrive through the corrected `doc/doc` locator; under the uncorrected one it was 11,454. This count is the as-built one (§6.7): the survey figure was 12,130, and the six files between them are the Flare output root `flogo-oracledb/1.2.1` ships as its `doc/` folder, which the owning-root skip hands back to the converter.

**Name matching is case-insensitive and separator-tolerant, and the separator is where this goes wrong.** The corpus spells these names with underscores, hyphens, dots *and spaces* (`tib_ems_relnotes.pdf`, `mft platform server v7.1 for windows release notes.pdf`, `tib_nimbus_9.1.0_licencing_doc.pdf` — note `licencing`). A first pass using `\b` as the boundary misrouted **1,753 release notes into `user-guides`**, because `_` is a word character and `\b` therefore does not match between `_` and `rel`. *Corrected 2026-09-15:* that figure counted the `pdf/` folder alone. The same anchor misses **1,108 further files in `doc/`**, where the consequence is different but no better — a readme filed as a reference document instead of release information — so the real exposure is **2,921 files**, and the regression test has to cover both folders rather than just the one the survey looked at. This is the same failure mode as the predecessor's substring skip-list (§3.9, `design.md` §6.3.1) seen from the other side: there the boundary was too loose, here too tight. The patterns are written out in `design.md` §10.4 and are tested against the observed spellings rather than the expected ones.

#### 6.2.2 The document doc-classes need an `index.md`, a `toc.yml` and a `metadata.yml` too

A copied PDF is not reachable. `online-help` gets navigation because Stage 6 synthesizes it from the source TOC; the three document doc-classes get nothing, so today a reader arriving at `user-guides/10-4-0/` sees a directory listing at best and a 404 at worst. **Every doc-class folder that receives at least one file also receives an `index.md`, a `toc.yml` and a `metadata.yml`**, built from the routed file list rather than from a source TOC — there is no source TOC to build from. The `metadata.yml` joined the pair when the AEM contract arrived (2026-09-10) and is the same one-key `csg-version` file `online-help` gets; a folder that receives no file still gets nothing.
...
2. **The PDF's Info-dictionary `/Title`**, for `user-guides`. Read over **all 5,007** de-duplicated files, not a sample: **70.8% usable (3,545), 27.9% blank (1,396), 1.2% junk (59), 7 files unreadable** — re-measured 2026-09-15 with `pypdf` alone, which is what ships; the tallies first recorded here (1,397 blank, 65 junk, 0 unreadable) were MuPDF's, and the difference is exactly the 7 files pypdf cannot parse. Usable values are genuine document titles — `'TIBCO® Data Virtualization SQL Server Adapter Guide'`, `'MFT Platform Server for Windows User Guide'`, `'TIBCO ActiveMatrix Adapter for IBM i Concepts'`.
...
**The junk filter is small and closed, so it is worth writing out.** Every junk title is **the authoring tool's source filename** — 65 of them in 37 distinct values as MuPDF read the corpus, **59 in 35** as pypdf does: 22 are `untitled`/`Untitled`, the rest end in `.book` (FrameMaker) or `.fm`, or begin `Microsoft Word - `. The six that differ are pypdf's unreadable files, `Microsoft Word - TIB_silver_mobile_install_guide.docx` and its kin, which the filter would have rejected anyway. Reject a title that is `untitled` case-insensitively, ends in `.book`/`.fm`/`.doc`/`.docx`/`.pdf`/`.indd`/`.mif`, or begins `Microsoft Word - `; those fall through to step 3. Nothing else needs rejecting — 0 titles are suspiciously short.
...
**Its `toc.yml` is the single-entry form too** (Phase 9, 2026-09-17) — one item at `index.md`, exactly as §6.2.2's three. This is the one case with a real argument for staying per-entry, and it was decided rather than swept up: these items are catalog rows, not files in the folder, and most name a ZIP that is not there, so unlike a PDF they never implied a page-per-entry. It collapses anyway, because one rule covering every generated `toc.yml` is worth more than a defensible exception nobody will remember. Everything this section argues for — the complete history, both availability states, the per-version link — is in `index.md` and unaffected.
...
**The links are real.** Re-measured 2026-09-15 through the shipped classifiers — `links.classify`, `links.is_topic`, `links.escapes`, `roots.owning_root` and `apiref.find_api_roots` with 6d's output-root guard: **3,194 relative references resolve into an API root, in 41 of 49 in-scope versions.** The distribution is nothing like the version counts.
...
**DITA is 81% of the traffic in 7% of the versions, and WebWorks is one link.** Both are worth stating rather than averaging away: the phase is justified by `amx-bpm` alone, and WebWorks earns its edit only because the check is the same three lines as the other three. A further 14 links in 3 `rtview` versions are **unreachable** — `rtview` detects as FrontPage, which is not in `CONVERTIBLE_ENGINES`, so those topics never become Markdown and have no link to rewrite.

**They did not survive conversion, which is why the rewrite could not be a Stage 7 pass.** A link into an API root is a topic-suffixed `.html` that is not in the engine's topic map, so each engine's `link()` fired `dangling_link` and returned `None` — the text kept, the link gone. A rewriter walking the placed docs tree would have found nothing to rewrite and would have reported success doing it: the §5.5.9 failure mode with the sign flipped. So the rewrite happens **while the engine is emitting**, which is a Stage 5 change that arrived in a Stage 6 phase.

**Two corrections to what 6d recorded, both in the direction of "smaller than I said".** 6d's ledger called this a change that "reopens the link path in all four engines and every link test they have", and estimated it at a phase's worth of work. That was wrong. The engines emit exactly what they emitted before; **one branch that returned `None` returns a URL instead**, and it is the same three lines in four files. Nor did anything need inventing for the address: `ConversionContext` already carried `api_roots`, `slug` and `version`; the driver already held the `ConfigManager` that knows `resources_tree_name()` and `publish_base_url()`; and `sync/apirefs.published_url()`, written and tested in 6d, already composed it. What was missing was only that the two halves had never been introduced. The second correction is the one that matters for reading the rest of this file: a cost estimate written at the end of a phase, about work the phase did not do, is the least reliable sentence in it.

**And one thing got *easier* by moving.** The Stage 7 plan carried a sub-bullet — the rewrite breaks 6b/6c's currency comparison, so rewrite in the staging directory and compare currency against the rewritten copy rather than against `output/`. It **disappears**. When conversion writes the URL, the converted tree on disk already contains it, so `sync` copies a finished file and its size-and-mtime comparison is untouched. That complexity was an artifact of doing the work in the wrong stage.

Five rules, all of them about where the answer is *not* computed:

- **The driver injects a resolved map; the engines never compose a URL.** `converter/driver.py:_api_urls` calls `sync/apirefs.url_map()` and hands the result to `ConversionContext.api_urls`, and the engines reach it through one accessor, `context.api_url(source, path, fragment)`. §10.7 assigned this rewrite to Stage 7 because conversion does not know the publishing layout, and it still does not — it is *told*, exactly as it is already told `api_roots` rather than walking for them.
- **The map is built through the same `select()` path Stage 7 publishes with**, so the folder that gets written and the URL that gets emitted come from one de-duplication, one naming rule and one collision fallback. Anything less and `tps/6.0.0`, whose whole version demotes to dashed names, gets links to folders that were never created. **Every root is a key, including the 19 de-duplicated away**, each mapping to its survivor's URL: a topic inside `rtview`'s self-nested extract links to the copy beside it, and that copy is exactly the one `select` drops.
- **One touchpoint, before the topic/asset split rather than inside either arm.** `flare.py`, `dita.py`, `docbook.py` and `webworks.py` each check immediately after `reference.resolvable`. Every one of the 3,194 measured references is a topic-suffixed page and `links.escapes()` fires zero times, but a rewrite that handled only the arm the corpus currently uses would be one repackaging away from silently dropping links again.
- **A target that is not on disk stays dangling, and a fragment survives.** The test is "resolves inside a published API tree **and** exists", which is `apiref.py`'s rule — a marker decides and a name never does — applied to the link side; emitting a URL for a page the copy will not contain trades a reported failure for a silent 404. Javadoc links are `#method-summary`-heavy, and a class page without its anchor is the wrong answer to the question the topic asked.
- **An unset `publish_base_url` emits the tree-rooted path with no scheme and no host**, decided 2026-09-15. The path is the part this tool can derive; a link missing only its prefix is fixable by search-and-replace when the host is known, and a link that was never emitted is not recoverable at all. A server-absolute `/en-us-…` was rejected: it works only if AEM serves both repositories from one host at content paths matching the repo names, which is an assumption about someone else's infrastructure.

**The currency key is the one trap in the phase.** `driver.py` keys conversion currency on the *package's* checksum, which is right for its own reason and wrong for this one: the package does not change when `publishing.yaml` does, so filling in `publish_base_url` would leave every converted tree reporting `CURRENT` with host-less links baked in — and the run would say so cheerfully. So the version's API URL prefix is recorded beside `convert_source_checksum` as `convert_api_prefix` and compared with it. It is recorded **empty for a version that emitted no URL**, which is 1,781 of the 1,822: setting a host must not reconvert the whole corpus to change the output of 41.

**`API_LINK_REWRITTEN`** is the phase's one new code — a note with a count, raised once per version. Nobody acts on one rewritten link and everybody wants the magnitude, and the failure it guards against is silent: a version with an API tree and **zero** rewritten links is either a product whose help genuinely never references its API (8 of the 46 versions converted) or a predicate that stopped matching, and only the count tells those apart. Register: 29 → 30.

**Measured through the shipped code, over the whole cache (2026-09-15).** Every in-scope version with an API tree was converted twice — once with the driver's URL map and once with an empty one — and the two runs reconcile exactly:

| | Links |
| :--- | ---: |
| In-scope references into an API root (46 convertible versions) | **3,180** |
| **Rewritten to a published URL**, in **38** versions | **2,854** |
| Flattened inside a `<code>` span before `link()` ran — see below | 324 |
| Target not on disk, left dangling as designed | 2 |

Nothing is unaccounted for. `TOPIC_LINK_DANGLING` falls by exactly 2,854 across the same runs, which is the check that matters: every URL emitted is a finding that stopped being raised. One version per engine, because the edit is in four files — `amx-bpm/4.2.0` (DITA) **1,289 of 1,290**, the one exception being the single reference whose target the package does not ship; `sfire-sfds/10.6.5` (DocBook) **36 of 36**; `bcedi/6.10.0` (WebWorks) **1 of 1**; `ems/10.4.0` (Flare) **8 of 89**, for the reason below.

**The one shortfall — closed by Phase 8, and left standing here because the number it names is what sized that phase.** What follows describes 6e's world; `planning.md` Phase 8 is what changed it. `ems/10.4.0`'s 8-of-89 below is now **86**, measured the same way through the same harness.

**A link inside a `<code>` span never reaches `link()`.** `transforms/markdown.py` renders `code`/`tt`/`kbd`/`samp` from their *text*, so `<code><a href="../api/javadoc/javax/jms/MessageListener.html">MessageListener</a></code>` becomes a bare code span and the reference is gone before any engine sees it. That is pre-existing behaviour in the shared walk, not something 6e introduced, and it is invisible in the findings register because no `dangling_link` is raised either — the link is not dropped, it is never classified. It costs **324 of the 3,180**, all of them in `ems`, whose developer guide writes every API cross-reference as a code span; **320 of the 324** are the anchor alone inside the span, which GFM expresses by inverting the nesting (``[`MessageListener`](url)``). It is left for its own change because it is a change to the walk every engine shares and every link passes through, not to the API-link path — 6e's scope is the rewrite, and the rewrite is correct for every reference that reaches it.

**And the API references are the small half of it.** Counted over every relative anchor in the same 96 versions (2026-09-16): **7,999 of 931,715 sit inside a code span, 7,761 of them alone in it.** The other 7,675 are ordinary topic-to-topic cross-references whose targets *are* converted — they would resolve if they reached `link()`, and they are lost the same silent way. It is one engine and very nearly one product: Flare 7,696, DocBook 224, DITA 79, WebWorks and the two non-convertible generators 0, with `ems` alone at **7,263**, about 1,800 per version. Which reframes the fix rather than merely sizing it: it is worth ~7,700 links, and its blast radius, though it sits in code every engine runs, lands almost entirely in four versions of one product.

**Both figures above undercount, and Phase 8 says by how much.** They count one site. There are six: each engine's `inline_override` maps a *class* to code and flattens it identically, and `<pre>` renders a fence the same way. Re-measured over all six across all 13 products — **13,126 swallowed references**, of which **8,162 are in an inline span** (7,863 with the anchor as the span's whole content, 299 sharing it) and **4,964 are inside a `<pre>`**. Phase 8 takes the first group and leaves the second, because a GFM fence has no syntax that can hold a link at all; the residue is counted per version as `CODE_LINK_FLATTENED` rather than left silent.

**What stays out, named rather than omitted.** §10.7's **class 2** — the 279 Flare references that escape into `doc/`, `pdf/` and the version root, bound for the *document* doc-classes — is not in 6e. Its destination is a relative path across doc-classes inside the same repository, which needs §10.4's router, and the router's answer is Stage 7 knowledge that conversion does not have and cannot be handed cheaply. `DOC_REFERENCE_MISSING` remains the one registered `Stage.SYNC` code no shipped path raises, and it is now the marker for that alone.

### 6.5 Navigation Synthesis (as built, Phase 6a)

`converter/navigation.py`. The four engines report a node list, a landing page and a support/legal pair per unit and decide none of `design.md` §10's three node rules; this is where those rules run, once, for all four, and where the per-unit lists become the one `toc.yml` a version publishes.

**It runs inside the conversion build, before the swap.** The node list exists only while the engine's `Unit` is in hand — nothing persists it — and the container pages it generates are Markdown documents that have to land in the staging tree, which stops existing the moment `utils/swap.py` runs. The alternative, writing the tree to `state.db` and synthesizing from a later command, stores a derived artifact whose only consumer runs milliseconds afterwards and makes a re-run decide whether to trust it.

Three things §10 did not say, measured over the predecessor's cache on 2026-09-12 and settled here:

- **A version is normally several units.** WebWorks ships more than one in **128 of 142 versions (90.1%)**, median 3 and up to 9; Flare in 15 of 157 (9.6%), SuiteHelp in 5 of 108 (4.6%), DocBook in 0 of 2. So one unit contributes its nodes to the version's top level directly and several get a node each — the same shape §5.3.3 already uses for `BookGroup`, where a level with one child is not navigation. **Unit order is the engine's**, never re-sorted: WebWorks returns books in `books.xml` declaration order, non-alphabetical in 71% of collections. A wrapper takes its unit's landing page as its own page, and the inner node pointing at that page is removed, so one page occupies one position.
- **The support/legal tail is positional, not deduplicated by content.** §10's "one legal node, not two" was written when Flare was the only engine and a version was one output root. **105 of 400 multi-book versions carry a legal page in more than one book, and in 83 of them the copies are not byte-identical** — but the difference is FrameMaker anchor ids (`<a name="113179">` against `<a name="113190">`) and the prose is the same, so a hash would keep all nine copies of one notice. The first unit in unit order supplies the version's tail; the others' copies leave the navigation as `NAV_NODE_DROPPED` and stay on disk, because a CSH identifier or an inbound link may still reach one. A duplicate that has children is **not** extracted — it is a section named like a tail page, and moving it would take its children with it.
- **A unit label needs a chain.** `Unit.title` first, filled by the engine because the engine is where the answer already is (WebWorks has `books.xml`'s book title, SuiteHelp has the homepage's `publication-title`); then the unit's landing-page title; then the directory stem with its separators opened out, since `tib_adas400_concepts` reads as nothing.

**Where no unit reports a landing page** — every WebWorks collection by design (§5.3.5), and a DITA TOC is a forest in 23 of 23 sampled doc-sets (§5.2.3) — the version gets a generated `index.md` rather than opening on whichever book sorted first. It is built last, so it links the tree as published, and it is titled from `books.xml`'s collection name where there is one, the catalog display name otherwise.

**Generated pages are ordinary `Document`s** with `generated: true` in frontmatter and no `source`, so `_render` supplies their frontmatter exactly once and they never enter the §9.3 output map — there is no source file to key them on, and no CSH identifier resolves to them. A generated page is placed in the deepest directory its descendants share, and never on top of a converted page.

**`toc.yml` is flattened in Python and substituted in Jinja, not recursed in Jinja.** Depth is the only part of the render that is not a straight substitution, and a recursive template that gets its own indentation wrong produces YAML that still parses — into a different tree. Every scalar goes through the same unconditional double-quote `csh.yml` uses (§5.4.2): a title carrying a colon is ordinary prose, not a mapping.

**`nav.yml` is gone and `meta.yml` is `metadata.yml`.** Nothing consumed `nav.yml`, nothing implemented it, and `toc.yml` was always the specified navigation artifact. `metadata.yml` carries `csg-product` at product level and `csg-version` — dotted, as the product names it, against the folder's dashed form — at version level, and nothing else; the ~11 fields the Phase-1 placeholder invented were deleted rather than kept alongside, because a guess shipped beside a contract reads as part of it.

### 6.6 Distribution Mechanics (as built, Phase 6b)

§6.1 fixes *where* a file goes and §6.2 fixes *which* file goes there. Neither says what happens on the second run, what the drop-down is built from, or what a version string that is not a version does to a directory name — and all three are decided by measurement rather than preference. Grounded 2026-09-15 against `config/versions.csv` and `config/products.csv`: 1,762 active versions on 458 products, spread over **nine `bu`/`family` pairs**, the largest being `tibco`/`general` at 707.

**The disk is the record for `version.yml`, and the catalog is the record for `archives/`.** These look contradictory and are the same rule applied to two different asymmetries. `archives/` is indexed from the catalog because the directory is a *subset* of the truth — ZIPs arrive on demand, so a folder holding two of forty versions would publish a history that looks complete (§6.2.3). A doc-class folder is the reverse: the catalog is the superset, because `sync` takes the same `--product` / `--version` scope options `convert` does and a scoped run touches one version out of thirty-eight. Building the drop-down from what the run wrote would rewrite a 38-entry list down to one and unlink the rest, reporting success. So `version.yml` is assembled by **reading the doc-class directory after the copy** and intersecting it with the catalog's active rows: a version synced last month keeps its row, a version whose folder was deleted loses it, and a `--version` run changes exactly one entry.

**The file is read before it is written, because the schema invites a human into it.** AEM's example permits a hand-written row with an absolute URL (`path: https://…`). Any row whose `path` does not resolve to a directory this doc-class holds is carried through verbatim, in its position relative to the generated block; a `version.yml` that fails to parse is left entirely alone and named in the report. Regenerating the file wholesale would make the first re-sync destroy the only copy of whatever was added by hand — and the tool cannot tell an intentional addition from a typo, so it must not adjudicate.

**Two active version strings are not path segments.** `Cloud™` (`tibco-cloud`) and `(iPaaS)` (`tibco-cloud-integration-ipaas`) are both convert-eligible, both in scope, and both their product's only active version, so neither can be routed around. The folder segment is therefore `slugify(version.replace('.', '-'))` — a no-op for 1,760 of 1,762 rows, and `cloud` / `ipaas` for the two — computed at this boundary and nowhere earlier, exactly as the dots-to-dashes conversion is (§6.2). The alternatives are worse in both directions: passing the raw string through puts a trademark glyph in a public URL path, and refusing to publish drops two in-scope products on the strength of an upstream parse artifact.

**One predicate names them, and it is the one that already exists.** `VERSION_NOT_NUMERIC` fires for any active row that is not `N(.N)*`, which is the same 20 rows that get reshaped or sorted last. Re-measured 2026-09-15, those 20 sit on **20 distinct products, one each — only 4 of them multi-version** (`spotfire-desktop`, `spotfire-enterprise-runtime-for-r-server-edition`, `spotfire-server`, `spotfire-statistics-services`). In the other 16 the artifact *is* the drop-down, so "sorts last" is invisible and the report line is the only place it surfaces. That is why it is a warning rather than a note, and why Phase 6b adds no finding code: `VERSION_UNDATED` covers the 13 dateless rows, and a rule needing a third code would be a rule nobody has measured.

**Dots-to-dashes is injective here, and the assertion is what keeps it so.** No product has two active versions that collapse to one segment. A future catalog carrying `6-2` beside `6.2` would publish one version on top of the other with no error, so the check belongs in §8.4's validation rather than in a comment.

**The wholesale replacement is one directory deep, and the rule is what it must not reach.** A version's doc-class folder is built in a staging sibling and swapped over the target with `utils/swap.py` — the same build-and-swap the converter uses, for the same virus-scanner reason. It must not reach `{product}/{doc-class}/version.yml`, `{product}/metadata.yml`, or any sibling version folder. A swap scoped to the doc-class rather than the version would delete the thirty-seven versions the run did not touch: the partial-sync failure again, one level up.

**Currency is compared, not recorded.** `sync` stores no fingerprint in `state.db` to decide whether a version is already current. The target directory is outside this tool's control — the `version.yml` rule above assumes a human edits it — so a stored hash would claim currency for a tree that no longer matches it. The comparison is size and mtime over the two trees' file lists, which works because the copy is `shutil.copy2`: a file DocuShift published carries the converted tree's mtime and a file somebody edited does not. A version with no converted output is one of the five outcomes the run reports, not an abort: `sync` ends with the same five-outcome table `download`, `extract` and `convert` do, because it runs over the same selection and a partially converted corpus is the normal state.

**Verified end to end against the real catalog, 2026-09-15.** Over the 1,389 convert-eligible versions on 422 products, `sync --all` wrote the expected **nine docs trees**, 422 product `metadata.yml` and 422 `version.yml`, and raised exactly the 15 `VERSION_NOT_NUMERIC` and 4 `VERSION_UNDATED` rows the selection contains — counted independently from the CSV, not read back from the run. Every one of the **317 published epoch-millisecond dates** rendered to the month the raw value decodes to. The largest drop-down (`tibco-cloud-integration-hybrid`, 33 rows) is in numeric order with its release dates monotonically descending alongside, which is the lexical-sort trap failing to appear. `spotfire-statistics-services` puts `Services` below `14.0.0` and `12.0.9`; `tibco-cloud` and `tibco-cloud-integration-ipaas` published as `/cloud` and `/ipaas`. A second `sync --all` reported all 1,389 already current. A `--version 3.12.0 --force` run over the 33-version product re-swapped one folder and left the other 32, the `version.yml` and a hand-written absolute-URL row exactly where they were — the partial-sync failure this section exists to prevent, measured rather than argued.

### 6.7 Document Placement (as built, Phase 6c)

`sync/router.py` and `sync/documents.py`, driven by `sync/distributor.py:sync_documents`. §6.2.1 fixes which doc-class a document reaches and §6.2.2 fixes what the folder gets beside it; this is what the sub-phase actually had to decide, and every item here is a place where the obvious implementation is measurably wrong.

**The documents come from `extract_path()`, not `output_path()`.** This is the one thing the three document doc-classes do not share with `online-help`, and it is what the whole sub-phase turns on: a version that never converted — no engine, a partial ZIP, an unsurveyed generator — can still ship eight PDFs and a readme. So one version is routinely `NO_OUTPUT` for `online-help` and `SYNCED` for `user-guides` in the same run, and the two drop-downs beneath one product legitimately list different version sets. 6b's `version.yml` assembly needed no change to produce that, because it already takes the present set from the directory listing rather than from the run.

**Everything else rides on 6b unchanged** — same staging sibling, same `utils/swap.py`, same one-directory-deep boundary, same five outcomes. The only new finding code is `DOCUMENT_UNREADABLE`, and it is the first code in the §7.5 register that was not there before the phase that raises it. That is the test the register's rule states in the other direction: needing a code the table lacks means the *rule* is new, and "a shipped PDF is damaged" genuinely is.

**A row per doc-class, and two kinds of absence.** A run reports one row per (version, doc-class) that had something to say, so a version appears up to four times in the outcome table. A missing extracted tree is one `NO_OUTPUT` row against the *version* — `doc_class` empty, naming `docushift extract` — because it is recoverable and it is not a fact about `user-guides`. A tree that is present and routes nothing gets no row: 156 of 1,822 versions are in that state, and reporting them would put 156 recoverable-looking failures in every full run. The same restraint applies upward: an unusable version string is named once by `sync_one`, and the product's two catalog findings are raised once over the union of published segments rather than once per doc-class, or a warning count would become a function of how many PDFs a package happened to ship.

**Currency is compared before anything is staged.** `online-help` copies the tree and then compares it, which is affordable for Markdown. Here the copy is hundreds of megabytes of PDFs and the comparison would follow thirteen minutes of re-opening them to build titles nobody will look at. So the check runs first, on the routed filename set plus the three rendered files, then size-and-mtime per file — the same `copy2` mtime `_identical` relies on. It rests on an inference worth stating: the rendered files are a pure function of the routed filenames and the PDFs' contents. A template change escapes it, and `--force` is the answer, as it is to a changed engine.

**The router's four corrections, each of which a plausible implementation gets wrong.** Both folders nest and both must be looked for at both depths (`V/doc/pdf` *and* `V/doc/doc`; the first written spec dropped the second and lost 676 files in 346 versions). `\b` is not a usable boundary, because `_` is a word character — 2,921 files misrouted across the two folders. Only files *directly inside* the folders are documents, which `doc/relnotes/` tests and confirms. And a file an engine output root owns is the converter's, which is the precedence `extractor/inventory.py` already applies.
...
| `docushift csh` | What help is on the shelf, and what moved? | The published tree on disk | `csh validate` only |

The fourth row arrived in Phase 7c and is a narrowing of the third, not a fourth source of truth: it reads the same tree through the same module (§7.6). It is separate because "which version lost this Help identifier" is a question somebody asks with a ticket in hand, and answering it should not require an 84-second link walk.
...
**Built, Phase 7b**, in `validation/`. The layout §6.2 describes as something `sync` writes, read back: `{target}/{tree}/{locale}/{slug}/{doc-class}/{version-dashed}/`, with `archives/` as a unit that has no version segment. The selectors (`--product`, `--version`, `--doc-class`) filter **directory names**, and `--version` accepts `10.4.0` or `10-4-0` because the dashed form is what is on disk and the dotted form is what a human has in hand.
...
### 7.6 What the `csh` group reads

**Built, Phase 7c**, as `docushift csh {list,report,validate}` over `validation/csh.py`. It takes `--target-dir` and the same three selectors `validate` takes, and **reads the catalog nowhere**.

**That is a deliberate override of `planning.md` §7.3's "coverage across a batch".** A batch is a `versions.csv` column, and `docushift report --run last --code CSH_UNRESOLVED` already answers "did CSH come through for the batch I just converted" out of the run's own findings. Building a second answer to that question is exactly the convergence §7.1 exists to prevent — it is `status` growing a problems section, one layer down. What had no reader at all was the shelf: which identifiers are published, where they point, and what changed since the version below. Three things follow from choosing the tree:

- The published tree is **cumulative**, where a batch is a moment. Cross-version regression needs two versions on disk at once, and a batch has no reason to contain both.
- The tree is **the upgrade path the customer travels**. The Help button that breaks is the one in the version somebody installs next, which is the next folder on the shelf, not the next row in a run.
- It needs **no `versions.csv` agreement**, so a target another machine synced can be checked — the same property §7.5 gives `validate`, for the same reason.

**The `-resources` sibling is skipped.** `transforms/csh.py` writes `csh.yml` into the version's Markdown output root, which `sync` places in the docs tree and nowhere else; the resources tree holds copied API-reference trees and the archive index, neither of which has a help map or ever will. Walking it would double `csh report`'s row count to print zeroes, and would build a folder index over a Javadoc frame set looking for frontmatter that is not there. An empty selection exits 1 by §7.4's second rule, and says which tree it reads so the exit is diagnosable.

| Subcommand | Question | Gates |
| :--- | :--- | :--- |
| `csh list` | What identifiers are published, and are their targets there? | No |
| `csh report` | How much of the shelf is mapped, and what changed between versions? | No |
| `csh validate` | Are the help maps correct? | **Yes**, on `error` |

`list --identifier` is the query the group exists for: a support engineer arrives with "the Help button for `Gateway.BusinessAgreements` is broken in 6.11.0" and this answers where it went across every published version in one call. The lookup is **byte-exact** — `GatewayInstances` and `gatewayInstances` are two live help targets in the corpus (`design.md` §9.1) — and a case-only near-miss is named rather than matched. Versions that have a map and do not carry the identifier are listed too, because "which versions lost it" is the actual question behind the ticket.

`report --since` prints the §7.6 comparison in full — dropped, added and retargeted, every identifier named. That is the half the finding cannot carry: `CSH_IDENTIFIER_DROPPED` is one row per version with the magnitude in its count, because 784 per-identifier rows over the corpus would be 376 from two products re-keying their help (`design.md` §8.7). The register carries the magnitude; the command carries the detail.

`csh validate` runs exactly the functions `docushift validate` runs — `validation/csh.py` holds the only implementation of the comparison in the tool, and two implementations would be two answers to "did this Help button survive". What it does differently is skip the link, anchor, asset and artifact passes, which is its whole independent value and the phase claims no more for it. It gates by §7.4's third rule and on nothing else: a value naming a file that is not there is an error, a dropped identifier is a warning and never fails a run.
