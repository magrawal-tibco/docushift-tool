...
- [ ] **Engine Detector** (`engines/detector.py`) — resolve the generator per version from extracted content and write it back to `versions.csv`:
  - Marker-file pass first (cheapest, least ambiguous): Flare `*.mcwebhelp`/`*.mclog`/`Skins/`/`Data/`/`MicroContent/`; **DITA (SDL) `GUID-*.html` filenames or `static/head.js` + `static/body.js`**; WebWorks `wwhelp/`/`wwhdata/`; **R help `snext.css`/`snextchm.css`**.
